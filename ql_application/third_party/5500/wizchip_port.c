...
static struct {
 ...
 void ql_spi_master_dma_tx_rx_init(ql_spi_message_s *spi_msg)
 {
         ql_spi_config_s spi_test_cfg;
         
         spi_test_cfg.clk=10000000;
         ...
         ql_spi_init(QL_SPI_ID0, &spi_test_cfg);
 ...
 static void ql_spi_write_read(uint8_t* wBuf, uint8_t* rBuf, uint16_t tx_len,uint16_t rx_len)
 {
         ...
         ql_spi_transfer(QL_SPI_ID0,&msg);
 ...
 static void wizchip_eint_isr(void *p) {
     //wz_log("eint");
