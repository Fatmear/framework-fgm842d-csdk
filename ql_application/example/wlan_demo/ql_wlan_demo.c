...
#define QL_WLAN_DEFAULT_IP         "192.168.19.1"
#define QL_WLAN_DEFAULT_GW         "192.168.19.1"
...
void ql_demo_sta_adv_app_init(char *oob_ssid,char *connect_key)
{
	...
	os_memset( &wNetConfigAdv, 0x0, sizeof(ql_network_InitTypeDef_adv_s) );
...
void ql_demo_wlan_app_init(VIF_ADDCFG_PTR cfg)
{
    ...
    if(cfg->wlan_role == QL_STATION) {
        if(cfg->adv == 1) {
        	os_memset( &networkadv_cfg, 0x0, sizeof(ql_network_InitTypeDef_adv_s) );
        	...
        	os_memset(&network_cfg, 0x0, sizeof(ql_network_init_s));
        ...
        network_cfg.wifi_retry_interval = 100;
        os_strcpy((char *)network_cfg.local_ip_addr, QL_WLAN_DEFAULT_IP);
        os_strcpy((char *)network_cfg.net_mask, QL_WLAN_DEFAULT_MASK);
        os_strcpy((char *)network_cfg.gateway_ip_addr, QL_WLAN_DEFAULT_GW);
        os_strcpy((char *)network_cfg.dns_server_ip_addr, QL_WLAN_DEFAULT_GW);
...
void ql_demo_state_app_init(void)
{
	...
	ql_wlan_log("sta: %d, softap: %d, b/g/n\r\n", ql_sta_ip_is_start(), ql_ap_ip_is_start());
...
void ql_demo_ip_app_init(void)
{
	...
	os_memset(&ipStatus, 0x0, sizeof(ql_ip_status_s));
