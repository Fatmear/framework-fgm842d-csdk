...

{
    ...
    if(strlen((char *)wifi_config_ssid))
	{

	    httpd_url_decode((char *)wifi_config_ssid, strlen((char *)wifi_config_ssid));
	    httpd_url_decode((char *)wifi_config_pwd, strlen((char *)wifi_config_pwd));
	...
	if(strlen((char *)uart_band))
	{

	    httpd_url_decode((char *)uart_band, strlen((char *)uart_band));
...
extern void ql_demo_softap_app_init(char *ap_ssid, char *ap_key);
 ...
 void ql_wlan_cfgnet_demo(void *data)
 {
 	uint32 band;
	uint8  test_band[4];
	...
	ql_demo_softap_app_init("FCM242D","12345678");
 	 ...
 	 while(1)
 	 {
		  ...
		  if(g_test_web_sem)
		  {
			  ql_rtos_semaphore_wait(g_test_web_sem, BEKEN_WAIT_FOREVER);
			   os_printf("get wifi config\n");
		  }

 		 if (wifi_config)
 		 {
			 
			 os_printf("wifi_config\n");

			 ql_demo_sta_app_init((char *)wifi_config_ssid, (char *)wifi_config_pwd);
		 ...
		 if (wifi_uart_config)
		 {
			...
			band = atoi((const char *)uart_band);
